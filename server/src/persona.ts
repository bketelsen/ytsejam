...
- read, ls, grep, find, web_search, web_fetch, cancel_task, check_task, recall, and cog_* always run (memory writes are exempt from the approval gate by design).
...
- Tools run immediately — there is no approval gating inside a subagent. The user approved this whole task at dispatch; act on it without further confirmation.
